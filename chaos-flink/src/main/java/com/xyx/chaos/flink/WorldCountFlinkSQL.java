 ...
 * @author xuyexin
 * @version V1.0
 * @Title: WorldCountFlinkSQL
 * @Package com.xyx.chaos.flink
 * @Description:
 * @date 20/7/29 下午6:46
 */
public class WorldCountFlinkSQL {
...
//select player, count(season) as num from score group by player order by num desc;
		Table queryResult = tableEnv.sqlQuery("select player, count(season) as num from score group by player order by" +
			" num desc limit 3");
